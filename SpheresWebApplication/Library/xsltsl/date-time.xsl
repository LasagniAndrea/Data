          ...
          <listitem>
            ...
            <variablelist>
              ...
              <varlistentry>
                <term>%h</term>
                <listitem>
                  <para>Hour in 24-hour format (0 - 23)</para>
                </listitem>
              </varlistentry>
  ...
  <xsl:template name="dt:get-day-of-the-week-name">
	...
	<xsl:choose>
		<xsl:when test = "$pCulture = 'fr-FR'">
			<xsl:choose>
				<xsl:when test="$day-of-the-week = 0">Dimanche</xsl:when>
				<xsl:when test="$day-of-the-week = 1">Lundi</xsl:when>
				<xsl:when test="$day-of-the-week = 2">Mardi</xsl:when>
				<xsl:when test="$day-of-the-week = 3">Mercredi</xsl:when>
				<xsl:when test="$day-of-the-week = 4">Jeudi</xsl:when>
				<xsl:when test="$day-of-the-week = 5">Vendredi</xsl:when>
				<xsl:when test="$day-of-the-week = 6">Samedi</xsl:when>
				<xsl:otherwise>error: <xsl:value-of select="$day-of-the-week"/></xsl:otherwise>
			</xsl:choose>
		</xsl:when>		
		<xsl:when test = "$pCulture = 'it-IT'">
			<xsl:choose>
				<xsl:when test="$day-of-the-week = 0">Domenica</xsl:when>
				<xsl:when test="$day-of-the-week = 1">Lunedi</xsl:when>
				<xsl:when test="$day-of-the-week = 2">Martedi</xsl:when>
				<xsl:when test="$day-of-the-week = 3">Mercoledi</xsl:when>
				<xsl:when test="$day-of-the-week = 4">Giovedi</xsl:when>
				<xsl:when test="$day-of-the-week = 5">Venerdi</xsl:when>
				<xsl:when test="$day-of-the-week = 6">Sabato</xsl:when>
				<xsl:otherwise>error: <xsl:value-of select="$day-of-the-week"/></xsl:otherwise>
			</xsl:choose>
		</xsl:when>		
		<xsl:otherwise>
  ...
  <xsl:template name="dt:get-day-of-the-week-abbreviation">
    ...
    <xsl:param name="pCulture" select="$pCurrentCulture"/>
 	...
 	<xsl:choose>
		<xsl:when test = "$pCulture = 'fr-FR'">
			<xsl:choose>
				<xsl:when test="$day-of-the-week = 0">Dim</xsl:when>
				<xsl:when test="$day-of-the-week = 1">Lun</xsl:when>
				<xsl:when test="$day-of-the-week = 2">Mar</xsl:when>
				<xsl:when test="$day-of-the-week = 3">Mer</xsl:when>
				<xsl:when test="$day-of-the-week = 4">Jeu</xsl:when>
				<xsl:when test="$day-of-the-week = 5">Ven</xsl:when>
				<xsl:when test="$day-of-the-week = 6">Sam</xsl:when>
				<xsl:otherwise>error: <xsl:value-of select="$day-of-the-week"/></xsl:otherwise>
			</xsl:choose>
		</xsl:when>		
		<xsl:when test = "$pCulture = 'it-IT'">
			<xsl:choose>
				<xsl:when test="$day-of-the-week = 0">Dom</xsl:when>
				<xsl:when test="$day-of-the-week = 1">Lun</xsl:when>
				<xsl:when test="$day-of-the-week = 2">Mar</xsl:when>
				<xsl:when test="$day-of-the-week = 3">Mer</xsl:when>
				<xsl:when test="$day-of-the-week = 4">Gio</xsl:when>
				<xsl:when test="$day-of-the-week = 5">Ven</xsl:when>
				<xsl:when test="$day-of-the-week = 6">Sab</xsl:when>
				<xsl:otherwise>error: <xsl:value-of select="$day-of-the-week"/></xsl:otherwise>
			</xsl:choose>
		</xsl:when>		
		<xsl:otherwise>
  ...
  <xsl:template name="dt:get-month-name">
    ...
    <xsl:param name="pCulture" select="$pCurrentCulture"/>
 	...
 	<xsl:choose>
		...
		<xsl:when test = "$pCulture = 'it-IT'">
			<xsl:choose>
				<xsl:when test="$month = 1">gennaio</xsl:when>
				<xsl:when test="$month = 2">febbraio</xsl:when>
				<xsl:when test="$month = 3">marzo</xsl:when>
				<xsl:when test="$month = 4">aprile</xsl:when>
				<xsl:when test="$month = 5">maggio</xsl:when>
				<xsl:when test="$month = 6">giugno</xsl:when>
				<xsl:when test="$month = 7">luglio</xsl:when>
				<xsl:when test="$month = 8">agosto</xsl:when>
				<xsl:when test="$month = 9">settembre</xsl:when>
  ...
  <xsl:template name="dt:get-month-abbreviation">
    ...
    <xsl:param name="pCulture" select="$pCurrentCulture"/>
 	...
 	<xsl:choose>
		<xsl:when test = "$pCulture = 'it-IT'">
			<xsl:choose>
				<xsl:when test="$month = 1">gen.</xsl:when>
				<xsl:when test="$month = 2">feb.</xsl:when>
				<xsl:when test="$month = 3">mar.</xsl:when>
				<xsl:when test="$month = 4">apr.</xsl:when>
				<xsl:when test="$month = 5">mag.</xsl:when>
				<xsl:when test="$month = 6">giu.</xsl:when>
				<xsl:when test="$month = 7">lug.</xsl:when>
				<xsl:when test="$month = 8">ago.</xsl:when>
				<xsl:when test="$month = 9">set.</xsl:when>
				<xsl:when test="$month = 10">ott.</xsl:when>
				<xsl:when test="$month = 11">nov.</xsl:when>
				<xsl:when test="$month = 12">dic.</xsl:when>
				<xsl:otherwise>error: <xsl:value-of select="$month"/></xsl:otherwise>
			</xsl:choose>
		</xsl:when>
    <xsl:when test = "$pCulture = 'fr-FR'">
      <xsl:choose>
        <xsl:when test="$month = 1">janv.</xsl:when>
        <xsl:when test="$month = 2">fév.</xsl:when>
        <xsl:when test="$month = 3">mars</xsl:when>
        <xsl:when test="$month = 4">avr.</xsl:when>
        <xsl:when test="$month = 5">mai</xsl:when>
        <xsl:when test="$month = 6">juin</xsl:when>
        <xsl:when test="$month = 7">juil.</xsl:when>
        <xsl:when test="$month = 8">août</xsl:when>
        <xsl:when test="$month = 9">sept.</xsl:when>
    ...
    <xsl:when test = "$pCulture = 'es-ES'">
      <xsl:choose>
        ...
        <xsl:when test="$month = 2">feb.</xsl:when>
        <xsl:when test="$month = 3">marzo</xsl:when>
        <xsl:when test="$month = 4">abr.</xsl:when>
