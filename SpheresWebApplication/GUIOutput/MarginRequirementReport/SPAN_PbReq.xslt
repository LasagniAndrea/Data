...
<xsl:stylesheet version="1.0"
  ...
  <!-- xslt includes -->
  <xsl:include href="SPAN_Common.xslt" />
  ...
  <xsl:template name="displayDetailMarginBody">
    ...
    <fo:table table-layout="fixed" width="100%">
      ...
      <xsl:for-each select ="//efs:span/efs:exchangecomplex/efs:groups/efs:group[efs:initialmarginbycurrency/efs:initialmargin[@curr=$pCurrency]]">

        <xsl:sort select="@name" />

        <!-- block exchangecomplex-->
        ...
        <!--block combinedcommodity-->
        <xsl:for-each select ="efs:combinedcommodities/efs:combinedcommodity[efs:initialmargin[@curr=$pCurrency]]">
          <xsl:sort select="@name" />
  ...
  <xsl:template name="displayDetailMarginRowValues">
    ...
    <xsl:variable name ="vExchangeComplexAcronym">
      <xsl:if test ="$pDisplayExchangeComplexAcronym=true()">
        <!-- L' ExchangeComplexAcronym est à lire sur le noeud "exchangecomplex" et non sur le noeud "group"-->
        ...
        <!--xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:call-template name ="getExchange">
              <xsl:with-param name ="pNode" select="ancestor::efs:group"/>
            </xsl:call-template>
          </xsl:otherwise>
        </xsl:choose-->
      </xsl:if>
