...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!--
  ==============================================================
  Summary : Test mapping for example files containing Fix trades on SPHERES®
  ==============================================================
  Version 3.4.0.0_4
  Date: 18/06/2013
  Author: RD
...
-->
  ...
  <xsl:template match="row">
    ...
    <row>
      ...
      <!-- Transcode the instrument identifier : P,C -> 'ExchangeTradedOption' ; F -> 'ExchangeTradedFuture' -->
      ...
      <xsl:variable name="vSelectIDM">
        <!--<xsl:call-template name="SelectIDM">
          ...
          <xsl:with-param name="pValues" select="data[@name='Exch']"/>
        </xsl:call-template>-->
        <SQL command="select">
          <xsl:attribute name="result">
            <xsl:value-of select="$gbnIDM"/>
          </xsl:attribute>
          select m.IDM
          from dbo.MARKET m
          where (m.IDENTIFIER = 'XDMI')
        </SQL>
      </xsl:variable>
      ...
      <!-- Transcode the PutCall external code : P -> 0 ; C -> 1 ; F -> null -->
      ...
      <!-- Count how many book/Acct matches the given EXTL_Acct  -->
      <xsl:variable name="vEnrichedAcct">
        <SQL command="select">
          ...
          from dbo.BOOK b
          where (b.IDENTIFIER = @EXTL_Acct)
          <!-- PL 20111124 where (b.EXTLLINK = @EXTL_Acct) -->
          <xsl:call-template name="SQLParamNodesBuilder">
            <xsl:with-param name="pNames" select="concat('EXTL_Acct',',','NOTFOUND',',','NOTUNIQUE')"/>
            <!-- PL 20111124 Temp xsl:with-param name="pValues" select="concat(data[@name='Acct'],',',$cNOTFOUND,',',$cNOTUNIQUE)"/ -->
            <xsl:with-param name="pValues" select="concat(data[@name='Client Info'],',',$cNOTFOUND,',',$cNOTUNIQUE)"/>
          </xsl:call-template>
        </SQL>
      </xsl:variable>
      ...
      <!-- Count how many Buyer-Seller/Pty.ID.27 matches the given EXTL_PtyID27  -->
      <xsl:variable name="vEnrichedPty.ID.27">
        <xsl:call-template name="SelectEnrichedPty.ID.27">
          <xsl:with-param name="pNames" select="concat('EXTL_PtyID27',',','NOTFOUND',',','NOTUNIQUE')"/>
          <xsl:with-param name="pValues" select="concat(data[@name='Pty.ID.27'],',',$cNOTFOUND,',',$cNOTUNIQUE)"/>
        </xsl:call-template>
      </xsl:variable>
      ...
      <!-- Count how many Clearer/Pty.ID.4 matches the given EXTL_PtyID4  -->
      ...
      <xsl:variable name="vTxnTm">
        <xsl:choose>
          <xsl:when test="string-length(data[@name='Contract Time']) = 3">
            <xsl:value-of select="concat(data[@name='BizDt'], 'T',
        substring(substring(concat('0', data[@name='Contract Time']),1,4),1,2),
        ':',
        substring(substring(concat('0', data[@name='Contract Time']),1,4),3,2),
        ':00')"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="concat(data[@name='BizDt'], 'T',
        substring(substring(data[@name='Contract Time'],1,4),1,2),
        ':',
        substring(substring(data[@name='Contract Time'],1,4),3,2),
        ':00')"/>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:variable>
      ...
      <xsl:variable name="vEnrichedSym">
        <xsl:call-template name="SelectEnrichedSym">
          <xsl:with-param name="pExternalLink" select="'IDENTIFIER'"/>
          <xsl:with-param name="pNames" select="concat('EXTL_Exch',',','SYMBOL',',','INSTRUMENT')"/>
          <xsl:with-param name="pValues" select="concat('XDMI',',',data[@name='Sym'],',',$vTCInstrumentIdentifier)"/>
      ...
      <xsl:variable name="vTrdCaptRptNode">
        <xsl:call-template name="BuildTrdCaptRptNode">
          <xsl:with-param name="TxnTm" select="$vTxnTm"/>
      ...
      <parameters>
        ...
        <xsl:call-template name="BuildGlobalParameter">
          <xsl:with-param name="pParamName" select="$gbnEnrichedPty.ID.27"/>
          <xsl:with-param name="pParamValue">
            <xsl:copy-of select="$vEnrichedPty.ID.27"/>
          </xsl:with-param>
        </xsl:call-template>
        <xsl:if test="$vExistsPty.ID.4 = 'true'">
          <xsl:call-template name="BuildGlobalParameter">
