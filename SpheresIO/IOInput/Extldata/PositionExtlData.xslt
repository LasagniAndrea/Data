...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="vEntityString">
    <xsl:choose>
      <xsl:when test="/iotask/parameters/parameter[@id='ENTITYIDENTIFIER_INPUT']">
        <xsl:value-of select="/iotask/parameters/parameter[@id='ENTITYIDENTIFIER_INPUT']"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:value-of select="'HPC'"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:variable>

  <xsl:variable name="vEXTLLINK_COLUMN">
    <xsl:choose>
  ...
  <xsl:variable name="vCONTRACTSYMBOL_COLUMN">
    <xsl:choose>
    ...
    <row>
      ...
      <xsl:variable name="vSelectIDM">
        <xsl:call-template name="SelectIDM">
          ...
          <xsl:with-param name="pExternalLink" select="$vEXTLLINK_COLUMN"/>
      ...
      <!-- Count how many Contrats/Sym match the given external link -->
      ...
      <xsl:variable name="vEnrichedSym">
        <xsl:call-template name="SelectEnrichedSymOrIdentifier">
          <xsl:with-param name="pExternalLink" select="$vEXTLLINK_COLUMN"/>
          <xsl:with-param name="pExternalSymbol" select="$vCONTRACTSYMBOL_COLUMN"/>
          <xsl:with-param name="pNames" select="concat('EXTL_Exch',',','SYMBOL',',','INSTRUMENT',',','NOTFOUND',',','NOTUNIQUE')"/>
          <xsl:with-param name="pValues" select="concat(data[@name='Exch'],',',data[@name='Sym'],',',$vTCInstrumentIdentifier,',',$cNOTFOUND,',',$cNOTUNIQUE)"/>
          <xsl:with-param name="pDtBusiness" select="$pDtBusinessParam"/>
        </xsl:call-template>
      </xsl:variable>

      <!-- Get the quantity type, if exists -->
      <xsl:variable name="vQtyTyp">
        <xsl:choose>
          <xsl:when test="data[@name='Typ']">
            <xsl:value-of select="data[@name='Typ']"/>
          </xsl:when>
          <xsl:otherwise>
      ...
      <xsl:variable name="vPosRptNode">
        <xsl:call-template name="BuildPosRptNode">
          ...
          <xsl:with-param name="QtyTyp" select="$vQtyTyp"/>
