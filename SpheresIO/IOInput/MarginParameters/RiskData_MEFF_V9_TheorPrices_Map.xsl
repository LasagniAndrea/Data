...
<!--
/*==============================================================
/* Name     : RiskData_MEFF_V9_TheorPrices_Map.xsl
/*==============================================================
/* Summary  : Import MEFF V9 Theoretical prices
/*==============================================================
/* Version  : 3.3
/* Date     : 20130411
/* Author   : PM
/* Comment  : Mapping de l'importation des prix théoriques
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:variable name="vXSLFileName">RiskData_MEFF_V9_TheorPrices_Map.xsl</xsl:variable>
  ...
  <xsl:template match="r">
    ...
    <r uc="true">
      <tbl n="IMMEFFTHEORPRICE_H" a="IU">
        ...
        <c n="NBPRICE" dku="true" t="i">
          <xsl:call-template name="AttribValueOrNull">
            <xsl:with-param name="pValue" select="./d[@n='NbTheoPrices']/@v"/>
          </xsl:call-template>
        </c>
        <!-- deltas -->
        <xsl:for-each select="./d[(substring(@n,1,1)='P') and (number(substring-after(@n,'P')) = substring-after(@n,'P'))]">
          <!-- Variable Scenario -->
          <xsl:variable name="vScenario">
            <xsl:value-of select="substring-after(@n,'P')"/>
          </xsl:variable>
          <c dku="true" t="dc" n="{concat('PRICE',$vScenario)}">
            <xsl:call-template name="AttribValueOrNull">
