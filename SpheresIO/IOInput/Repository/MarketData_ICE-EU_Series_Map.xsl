...
<!--
==============================================================================================
Summary : ICE-EU - RISKDATA
File    : MarketData_ICE-EU_Series_Map.xsl
==============================================================================================
==============================================================================================
Version : v1.0.0.0                                           
Date    : 20171018                                           
Author  : PLA
==============================================================================================
Version: v6.0.6488   Date: 20171018   Author: PLA
Comment: [23504] - Import: MiFID II: ISIN codes for series [ICE Futures Europe] (New feature)
==============================================================================================
Version: v6.0.6537   Date: 20171124   Author: PLA
Comment: [23504] - Correction on MaturityMonthYear variable
==============================================================================================
-->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template name="rowStream">
    ...
    <!-- ================================================================================ -->
    <xsl:variable name="vBusiness_Date">
      <xsl:value-of select="normalize-space(data[@name='BUSINESS_DATE'])"/>
    </xsl:variable>

    <xsl:variable name="vContractSymbol">
      <xsl:value-of select="normalize-space(data[@name='COMMODITY_CODE'])"/>
    </xsl:variable>

    <xsl:variable name="vExchangeSymbol">
      <xsl:value-of select="normalize-space(data[@name='EXCHANGE_CODE'])"/>
    </xsl:variable>

    <!-- Source      : FUT=Future Contract; OOF=Option Contract -->
    <xsl:variable name="vCategory">
      <xsl:value-of select="normalize-space(data[@name='SECURITY_TYPE'])"/>
    </xsl:variable>

    <!-- Source      : F=(Space); C=Call; P=Put -->
    <xsl:variable name="vPutCall">
      <xsl:call-template name="CallPut">
        <xsl:with-param name="pCallPut">
          <xsl:value-of select="normalize-space(data[@name='OPTION_TYPE'])"/>
        </xsl:with-param>
      </xsl:call-template>
    </xsl:variable>

    <xsl:variable name="vStrike">
      <xsl:value-of select="normalize-space(data[@name='STRIKE'])"/>
    </xsl:variable>

    <!-- Source      : 20171000 or 20171215-->
    <xsl:variable name="vMaturityMonthYear">
      <xsl:value-of select="normalize-space(data[@name='CONTRACT_PERIOD'])"/>
    </xsl:variable>
    <xsl:variable name="vMaturityDay">
      <xsl:value-of select="substring($vMaturityMonthYear, 7, 2)"/>
    </xsl:variable>
    <xsl:variable name="vMaturityMonthYearDay">
      <xsl:if test="$vMaturityDay = '00'">
        <xsl:copy-of select="substring($vMaturityMonthYear, 1, 6)"/>
      </xsl:if>
      <xsl:if test="$vMaturityDay != '00'">
        <xsl:copy-of select="$vMaturityMonthYear"/>
      </xsl:if>
    </xsl:variable>

    <xsl:variable name="vMarketID">
      <xsl:value-of select="normalize-space(data[@name='MARKET_ID'])"/>
    </xsl:variable>

    <xsl:variable name="vAssetISINCode">
      <xsl:value-of select="normalize-space(data[@name='ISIN'])"/>
    </xsl:variable>

    <xsl:variable name="vFISN">
      <xsl:value-of select="normalize-space(data[@name='FISN'])"/>
    </xsl:variable>

    <xsl:variable name="vCFI">
      <xsl:value-of select="normalize-space(data[@name='CFI'])"/>
    </xsl:variable>
      ...
      <table name="ASSET_ETD" action="U">
        <column name="IDASSET" datakey="true" datatype="integer">
          <SQL command="select" cache="true" result="IDASSET">
            ...
            inner join dbo.MARKET m on m.IDM=dc.IDM and (m.IDENTIFIER=@EXCHANGESYMBOL)
            ...
            where (dc.CATEGORY=@CATEGORY) and (dc.CATEGORY='F' or (asset.STRIKEPRICE=@STRIKEPRICE and asset.PUTCALL=@PUTCALL))
            ...
            <Param name="MATURITYMONTHYEAR" datatype="string">
              <xsl:value-of select="$vMaturityMonthYearDay"/>
            </Param>
            <Param name="CATEGORY" datatype="string">
              <xsl:choose>
                <xsl:when test="$vCategory='FUT'">
