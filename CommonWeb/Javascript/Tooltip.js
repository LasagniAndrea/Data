...
var ttWidth       = 200;
var ttTextInRowMasterTable = false;
var ttCssDataGrid = "Tooltip";
var ttCssDiv      = "";
...
tt_chrome = (tt_n.indexOf("chrome") != -1),
...
tt_n6 = (!tt_chrome && !tt_op && document.defaultView && typeof document.defaultView.getComputedStyle != tt_u),
...
{
	var t_opa     = (typeof tt.T_OPACITY != tt_u)? tt.T_OPACITY : ttOpacity,
	t_padd        = (typeof tt.T_PADDING != tt_u)? tt.T_PADDING : ttPadding,
	t_shc         = (typeof tt.T_SHADOWCOLOR != tt_u)? tt.T_SHADOWCOLOR : (ttShadowColor || 0),
	t_shw         = (typeof tt.T_SHADOWWIDTH != tt_u)? tt.T_SHADOWWIDTH : (ttShadowWidth || 0),
	t_tit         = (typeof tt.T_TITLE != tt_u)? tt.T_TITLE : "",
	t_w           = (typeof tt.T_WIDTH != tt_u)? tt.T_WIDTH  : ttWidth,
	t_cssDiv      = (typeof tt.T_CSSDIV != tt_u)? tt.T_CSSDIV : ttCssDiv;
	t_cssDataGrid = (typeof tt.T_CSSDATAGRID != tt_u)? tt.T_CSSDATAGRID : ttCssDataGrid;
	
	t_textInRowMasterTable  = (typeof tt.T_TEXTINROWMASTERTABLE != tt_u)? tt.T_TEXTINROWMASTERTABLE : ttTextInRowMasterTable;
	if(t_shc || t_shw)
	{
		t_shc = t_shc || "#d3d3d3";
		t_shw = t_shw || 3;
	...
	var t_optx = tt_ie? 'filter:Alpha(opacity='+t_opa+')' : 'opacity:'+(t_opa/100.0) + ';';
	//var t_optx = (tt_n4? '' : tt_n6? ('-moz-opacity:'+(t_opa/100.0)) : tt_ie? ('filter:Alpha(opacity='+t_opa+')') : ('opacity:'+(t_opa/100.0))) + ';';
	
	var t_y = '<div id="' + t_id + '"';
	if (t_cssDiv)
	    t_y += ' class="' + t_cssDiv + '" style="';
	else    
	    t_y += ' style="position:absolute;z-index:1010;';
    //t_y += 'left:0px;top:0px;width:'+(parseInt(t_w)+ parseInt(t_shw))+'px;visibility:'+(tt_n4? 'hide' : 'hidden') + ';' + t_optx + '">';		    
    t_y += 'left:0px;top:0px;width:'+(parseInt(t_w)+ parseInt(t_shw))+'px;display:'+(tt_n4? 'none' : 'none') + ';' + t_optx + '">';		    
	    
	if (t_cssDataGrid)
	{
    	t_y += '<table cellpadding="0" cellspacing="0" class="' + t_cssDataGrid + '" style="border-collapse:collapse;width:' + t_w + 'px;">';
	    if(t_tit)
	    {
		    t_y += '<tr class="' + t_cssDataGrid + '_HeaderStyle">';
		    t_y += '<td style="padding-left:3px;padding-right:3px;">' + t_tit + '   </td></tr>';
	    }
	    if ("true" == t_textInRowMasterTable)
	    {
            t_y += txt;	    
        }
        else
        {
	        t_y += '<tr class="' + t_cssDataGrid + '_ItemStyle">';
	        t_y += '<td>' + txt + '</td>';
	        t_y += '</tr>';
	    }
	    t_y += '</table>';
	...
	if(t_shw)
	{
		...
		{
			t_optx = tt_ie? 'filter:Alpha(opacity=80);' : 'opacity:0.80;';
			t_y += '<div id="'+t_id+'R" style="position:absolute;background:'+t_shc+';left:'+t_w+'px;top:'+t_spct+'px;width:'+t_shw+'px;height:1px;overflow:hidden;'+t_optx+'"></div>' +
				'<div style="position:relative;background:'+t_shc+';left:'+t_spct+'px;top:0px;width:'+(parseInt(t_w)- parseInt(t_spct))+'px;height:'+t_shw+'px;overflow:hidden;'+t_optx+'"></div>';
...
function tt_ShowDiv(t_x)
{
	...
	if(tt_n4) tt_obj.display = t_x? 'block' : 'none';
	else tt_obj.style.display = t_x? 'block' : 'none';
	tt_act = t_x;
}
...
function tt_Init()
{
	...
	{
	    --i;
		tags = tt_ie? (document.body.all.tags(tt_tags[i]) || 1)
	...
	if(document.getElementById("TTiEiFrM")) tt_ifrm = document.getElementById("TTiEiFrM");
}
tt_Init();//]]>
</script>
