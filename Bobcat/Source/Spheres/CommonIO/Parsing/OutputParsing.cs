...
namespace EFS.Common.IO
{
    ...
    /// 
    /// </summary>
    // PM 20180219 [23824] Déplacée à partir de EFS.SpheresIO (OutputParsing.cs)
    public class OutputParsing : IOParsing
    {
        ...
        //public OutputParsing(string pIdOutput, Task pTask) : base(pIdOutput, pTask)
        public OutputParsing(string pIdOutput, IIOTaskLaunching pTask)
        ...
        public Cst.ErrLevel MakeLine(IOTaskDetInOutFileRow pRow, ref string pLine, bool pIsTitle)
        {
            ...
            else
            {
                ...
                Logger.Log(new LoggerData(LogLevelEnum.Info, rowMsg + " not writted"));
            ...
            if (ret == Cst.ErrLevel.SUCCESS)
            {
                ...
                for (int parsingNumber = 0; parsingNumber < ParsingCount; parsingNumber++)
                {
                    ...
                    if (ParsingDetCount == 0)
                    {
                        ...
                        m_Task.SetErrorWarning(ProcessStateTools.StatusErrorEnum);
                       
                        
                        Logger.Log(new LoggerData(LogLevelEnum.Error, "(" + rowMsg + ") : no details specified", 0, new LogParam("Parsing " + m_IdParsing)));
                    ...
                    if (ret == Cst.ErrLevel.SUCCESS)
                    {
                        for (int parsingDetNumber = 0; parsingDetNumber < ParsingDetCount; parsingDetNumber++)
                        {
                            ...
                            if (StrFunc.IsFilled(errMsg))
                            {
                                ...
                                Logger.Log(new LoggerData(LogLevelEnum.Error, errMsg));
                            }
                            #endregion
                            //
                            if (isRowRejected)
        ...
        private bool RowIsToWrite(IOTaskDetInOutFileRow pRow)
        {
            ...
            if (ArrFunc.IsFilled(drAllParsings))
            {
                ...
                foreach (DataRow rowParsing in drAllParsings)
                {
                    ...
                    string idIOParsing = (Convert.IsDBNull(rowParsing["IDIOPARSING"]) ? string.Empty : rowParsing["IDIOPARSING"].ToString());
                    //
        ...
        private void WriteData(ref string pLine, bool pIsLastData)
        {
            ...
            if (m_DataStart > 0)
            {
                ...
                {
                    string messageValue = "Check your parsing parameters, " + messageStart + " is overlying other data";
                    string oldSub;
                    //
                ...
                {
                    int fillLength = m_DataStart - pLine.Length - 1;
                    //
        ...
        public static string GetDataValueFromRow(IIOTaskLaunching pTask, IOTaskDetInOutFileRow pRow, string pDataName, ref bool pDataFound)
        {
        ...
        public static string GetDataValueFromRow(IIOTaskLaunching pTask, IOTaskDetInOutFileRow pRow, string pDataName, ref bool pDataFound, ref bool pIsRejected)
        {
            ...
            for (int i = 0; i < pRow.data.Length; i++)
            {
                if (pRow.data[i].name == pDataName)
                {
                    ...
                    ProcessMapping.GetMappedDataValue(pTask, pTask.SetErrorWarning, pRow.data[i], null, IOCommonTools.SqlAction.NA, ref dataValue, ref resultAction, messageValue);
                    // PM 20180219 [23824] IOTools => IOCommonTools
                    dataValue = IOCommonTools.SetDataRowToDynamicString(dataValue, pTask, pRow, false);
