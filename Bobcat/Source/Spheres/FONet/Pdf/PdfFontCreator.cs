    ...
    internal sealed class PdfFontCreator
    {
        ...
        public PdfFont MakeFont(string pdfFontID, Font font)
        {
            ...
            else
            {
                ...
                if (realMetrics is Base14Font _base14)
                {
                    pdfFont = CreateBase14Font(pdfFontID, _base14);

                }
                else if (realMetrics is TrueTypeFont ttf) // TrueTypeFont restricted to the WinAnsiEncoding scheme 
            ...
            string pdfFontID, Font font, CIDFont cidFont)
        {
            ...
            PdfCIDSystemInfo pdfCidSystemInfo = new PdfCIDSystemInfo(cidFont.Registry, cidFont.Ordering, cidFont.Supplement);

            PdfCIDFont pdfCidFont = new PdfCIDFont(NextObjectId(), PdfFontSubTypeEnum.CIDFontType2, font.FontName)
            {
                SystemInfo = pdfCidSystemInfo,
                Descriptor = pdfDescriptor,
                DefaultWidth = new PdfNumeric(cidFont.DefaultWidth),
                Widths = cidFont.WArray
            };
            ...
            PdfType0Font pdfFont = new PdfType0Font(NextObjectId(), pdfFontID, font.FontName)
            {
                Encoding = new PdfName(cidFont.Encoding),
        ...
        {
            PdfType1Font type1Font = new PdfType1Font(NextObjectId(), pdfFontID, base14.FontName)
            {
            ...
            string pdfFontID, Font font, TrueTypeFont ttf)
        {
            ...
            PdfTrueTypeFont pdfFont = new PdfTrueTypeFont(NextObjectId(), pdfFontID, font.FontName)
            {
                Encoding = new PdfName("WinAnsiEncoding"),
                Descriptor = pdfDescriptor,
                FirstChar = new PdfNumeric(ttf.FirstChar),
                LastChar = new PdfNumeric(ttf.LastChar),
                Widths = ttf.Array
            };
        ...
        {
            if (font is ProxyFont font1)
            {
                return font1.RealFont;
            }
            else
            {
                return font;
        ...
        private PdfFontDescriptor MakeFontDescriptor(string fontName, IFontMetric metrics)
        {
            ...
            PdfFontDescriptor pdfDescriptor = new PdfFontDescriptor(fontName, NextObjectId())
            {
                Ascent = new PdfNumeric(metrics.Ascender),
                CapHeight = new PdfNumeric(metrics.CapHeight),
                Descent = new PdfNumeric(metrics.Descender),
                Flags = new PdfNumeric(descriptor.Flags),
                ItalicAngle = new PdfNumeric(descriptor.ItalicAngle),
                StemV = new PdfNumeric(descriptor.StemV)
            };
