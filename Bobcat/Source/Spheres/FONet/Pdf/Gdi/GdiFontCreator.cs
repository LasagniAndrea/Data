...
namespace Fonet.Pdf.Gdi {
    ...
    public class GdiFontCreator {
        ...
        private readonly TrueTypeHeader header;
        private readonly MemoryStream ms;
        private readonly FontFileStream fs;
        ...
        private byte[] ReadTableData(string tableName)
        {
            ...
            uint size = NativeMethods.GetFontData(dc.Handle, tag, 0, null, 0);

            byte[] data = new byte[size];
            uint rv = NativeMethods.GetFontData(dc.Handle, tag, 0, data, (uint)data.Length);
