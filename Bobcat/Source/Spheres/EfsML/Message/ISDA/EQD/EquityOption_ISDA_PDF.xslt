...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:dt="http://xsltsl.org/date-time" xmlns:fo="http://www.w3.org/1999/XSL/Format" version="1.0">
	...
	==============================================================
	File    : EquityOption_ISDA_PDF.xslt
	Date    : 20.05.2009
	Author  : Gianmario SERIO
	Version : 2.3.0.3_1
	Description:
		...
		<fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format">
			...
			<fo:page-sequence master-reference="A4" initial-page-number="1" font-family="{$varFontFamily}">
				...
				<fo:flow flow-name="A4-body">
					...
					<xsl:call-template name="displayEqdConfirmationText"></xsl:call-template>

					<!-- Call template Display Swaption Terms (notional amount, trade date, etc.) -->
					<xsl:call-template name="displayEqdTerms">
					...
					<xsl:call-template name="displayEqdExerciseProcedure">
						<xsl:with-param name="pProduct" select="//dataDocument/trade/equityOption/equityExercise"/>
					</xsl:call-template>
					<xsl:call-template name="displayBlockBreakline"/>

					<!-- Call template Settlement Terms -->
					<xsl:call-template name="displayEqdSettlementTerms"/>
					<xsl:call-template name="displayBlockBreakline"/>
