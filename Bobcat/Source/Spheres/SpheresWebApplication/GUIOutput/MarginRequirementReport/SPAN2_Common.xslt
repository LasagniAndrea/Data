...
<xsl:stylesheet version="1.0"
  ...
  <xsl:variable name ="vSpan2Core" select ="'SPAN 2 CORE'"/>
  <xsl:variable name ="vSpan2Software" select ="'SPAN 2 SOFTWARE'"/>
    ...
    <xsl:variable name ="vAmount">
      <xsl:choose>
        <xsl:when test ="$vGlobalRequirementType = ''">
          <xsl:value-of select ="sum(//efs:groups/efs:group/efs:riskInitialByCurrency/efs:riskInitial[@curr=$pCurrency]/@amount)"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of select ="sum(//efs:groups/efs:group/efs:riskMaintenanceByCurrency/efs:riskMaintenance[@curr=$pCurrency]/@amount)"/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    <xsl:value-of select ="$vAmount"/>
      ...
      <xsl:choose>
        <xsl:when test ="$pIsGroupNode=true()">
          <xsl:choose>
            ...
            <xsl:otherwise>
              <xsl:call-template name ="formatMoney">
                <xsl:with-param name ="pAmount" select ="sum($pNode/efs:pods/efs:pod/efs:lov[@curr=$pCurrency]/@amount)"/>
              </xsl:call-template>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:when>
      ...
      <xsl:choose>
        <xsl:when test ="$pIsGroupNode=true()">
          <xsl:choose>
            ...
            <xsl:otherwise>
              <xsl:call-template name ="formatMoney">
                <xsl:with-param name ="pAmount" select ="sum($pNode/efs:pods/efs:pod/efs:sov[@curr=$pCurrency]/@amount)"/>
              </xsl:call-template>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:when>
      ...
      <xsl:choose>
        <xsl:when test ="$vGlobalRequirementType = '' or $vGlobalRequirementType = $vLabelInit">
          <xsl:choose>
            <xsl:when test ="$pIsGroupNode=true()">
              <xsl:call-template name ="formatMoney">
                <xsl:with-param name ="pAmount" select ="$pNode/efs:riskInitialByCurrency/efs:riskInitial[@curr=$pCurrency]/@amount"/>
              </xsl:call-template>
            </xsl:when>
            <xsl:otherwise>
              <xsl:call-template name ="formatMoney">
                <xsl:with-param name ="pAmount" select ="$pNode/efs:riskInitial[@curr=$pCurrency]/@amount"/>
              </xsl:call-template>
            </xsl:otherwise>
          </xsl:choose>
        </xsl:when>
          ...
          <xsl:choose>
            <xsl:when test ="$pDetailRequirementType = $vLabelMaint">
              <xsl:choose>
                <xsl:when test ="$pIsGroupNode=true()">
                  <xsl:call-template name ="formatMoney">
                    <xsl:with-param name ="pAmount" select ="$pNode/efs:riskMaintenanceByCurrency/efs:riskMaintenance[@curr=$pCurrency]/@amount"/>
                  </xsl:call-template>
                </xsl:when>
                <xsl:otherwise>
                  <xsl:call-template name ="formatMoney">
                    <xsl:with-param name ="pAmount" select ="$pNode/efs:riskMaintenance[@curr=$pCurrency]/@amount"/>
                  </xsl:call-template>
                </xsl:otherwise>
              </xsl:choose>
            </xsl:when>
