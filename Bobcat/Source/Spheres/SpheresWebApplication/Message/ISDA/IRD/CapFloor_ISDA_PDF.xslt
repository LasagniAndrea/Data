...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:dt="http://xsltsl.org/date-time" xmlns:fo="http://www.w3.org/1999/XSL/Format" version="1.0">
  ...
  <!--
	==============================================================
	Summary : CapFloor_ISDA_PDF.xslt
	==============================================================
	Revision: 3
	Date    : 20100106
	...
	Update  : Add variable varAnnexSectionTextLongColumnWidthCM in variables region
	          ...
	          Rename 'pIsCollar' parameter in 'pCapFloorType' 
	          Add displayAdditionalInformation
	==============================================================	
	File    : CapFloor ISDA PDF
	Date    : 20.05.2009
	Author  : Gianmario SERIO
	Version : 2.3.0.3_1
	Description:
  ...
  <xsl:import href="CapFloor_ISDA_Business.xslt"/>
      ...
      <fo:page-sequence master-reference="A4" initial-page-number="1" font-family="{$varFontFamily}">
        ...
        <fo:flow flow-name="A4-body">
          ...
          <xsl:call-template name="displayIrdProductTerms">
            ...
            <xsl:with-param name="pStreams"             select="$capFloorStream"/>
          ...
          <xsl:call-template name="handleIrdStreams">
            <xsl:with-param name="pStreams" select="$capFloorStream"/>
            <xsl:with-param name="pCapFloorType" select="$varCapFloorType"/>
            <xsl:with-param name="pIsNotionalDifferent" select="$isNotionalDifferent"/>
            <xsl:with-param name="pIsScheduledNominalDisplayInAnnex" select="$isScheduledNominalDisplayInAnnex"/>
          ...
          <xsl:if test="//dataDocument/trade/capFloor/additionalPayment">
            <xsl:call-template name="displayAdditionalPayment">
              <xsl:with-param name="pInstrument" select="//dataDocument/trade/capFloor"/>
            </xsl:call-template>
          </xsl:if>
          ...
          <xsl:if test="$isScheduledNominalDisplayInAnnex = 'true'">
            <xsl:call-template name="displayAnnex">
              <xsl:with-param name="pIsNotionalDifferent" select="$isNotionalDifferent"/>
              <xsl:with-param name="pStream"             select="$capFloorStream"/>
            </xsl:call-template>
          </xsl:if>
