...
namespace EFS.DALOracleManaged
{
	/// <summary>
	...
	public sealed class OraHelper
	{
		...
		/// EG 20240205 [XXXXX] Gestion Bug avec Oracle Managé sur DataAdapter (Cas UpdateBatchSize <>1)
				...
				try
				{
					...
					using (OracleCommand cmd = new OracleCommand())
					{
						...
						using (OracleDataAdapter da = new OracleDataAdapter(cmd))
						{
							...
							{
								// EG 20240205 [XXXXX] Gestion Bug avec Oracle Managé sur DataAdapter (Cas UpdateBatchSize <>1 seulement)
								da.UpdateBatchSize = updateBatchSize;
								if (da.UpdateBatchSize != 1)
								{
		...
		// EG 20240723 [10000341][26115] CORPORATE ACTION : Correction(ORA-02291: integrity constraint (FK_DRVATTRIB_DRVCONTRACT) violated - parent key not found)
		// -> par clonage de l'instruction InsertCommand !!!
		public static int Update(OracleDataAdapter pDa, DataTable pDataTable, RowUpdating pRowUpdating, RowUpdated pRowUpdated)
		{
			...
			{
				pDa.InsertCommand = (OracleCommand)cb.GetInsertCommand().Clone();
				// EG 20240205 Le try/Catch n'est plus appliqué (problème persistant quand UpdateBatchSize<>1 est utilisé, pas le cas ici)
