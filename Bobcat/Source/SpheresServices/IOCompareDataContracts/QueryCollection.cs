...
namespace IOCompareCommon
{
        ...
        where TInternal : ISpheresComparer
    {
        ...
        private string m_CS;
        private bool m_IsMatchingOnlyClearer;
        ...
        private static string QueryInternalTradesInPosition()
        {
            ...
            return @"
                ...
                alloc.IDT               as IDDATA,
                ''						as COLLECTEDVALUES,
                TRADE.TRADEXML.value('(efs:EfsML/trade/efs:exchangeTradedDerivative/fixml:FIXML/fixml:TrdCaptRpt/@TxnTm)[1]','UT_ENUM_OPTIONAL') 
                                        as TXNTM,
                alloc.SIDE              as SIDE,
                asset.CONTRACTIDENTIFIER	as SYM,
                asset.MATURITYMONTHYEAR as MMY,
                asset.STRIKEPRICE		as STRKPX,
                TRADE.TRADEXML.value('(efs:EfsML/trade/efs:exchangeTradedDerivative/fixml:FIXML/fixml:TrdCaptRpt/@LastPx)[1]','UT_PRICE') 
						                as LASTPX,		
                alloc.ISO10383_ALPHA4	as EXCH, 
                asset.PUTCALL			as PUTCALL,
                b.IDENTIFIER 
						                as ACCT,
                TRADE.TRADEXML.value('(efs:EfsML/trade/efs:exchangeTradedDerivative/fixml:FIXML/fixml:TrdCaptRpt/fixml:RptSide/@AcctTyp)[1]','UT_ENUMCHAR_OPTIONAL') 
						                as ACCTTYP,	
                TRADE.TRADEXML.value('(efs:EfsML/trade/efs:exchangeTradedDerivative/fixml:FIXML/fixml:TrdCaptRpt/fixml:RptSide/@PosEfct)[1]','UT_ENUMCHAR_OPTIONAL') 
						                as POSEFCT,																
                alloc.QTY 
                - isnull(pab.QTY,0) 
                - isnull(pas.QTY,0)  
