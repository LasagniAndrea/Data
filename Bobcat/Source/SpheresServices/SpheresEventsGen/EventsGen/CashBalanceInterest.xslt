...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  <xsl:template mode="product" match="item[@name='CashBalanceInterest']">
    <xsl:param name="pPosition" select="0"/>
    <cashBalanceInterest>

      <!-- Product -->
      <keyGroup name="CashBalanceInterest" key="PRD"/>
      ...
      <!-- Stream -->
      <cashBalanceInterestStream>
        <xsl:call-template name="Stream">
          <xsl:with-param name="pProduct" select="'CashBalanceInterest'"/>
        </xsl:call-template>
      </cashBalanceInterestStream>
