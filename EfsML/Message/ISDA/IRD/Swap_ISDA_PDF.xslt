...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                xmlns:dt="http://xsltsl.org/date-time" xmlns:fo="http://www.w3.org/1999/XSL/Format" version="1.0">

  <!--
	==============================================================
	Summary : Swap ISDA PDF
	==============================================================
	Revision: 4
	Date    : 14/02/2011
  ...
  Revision: 3
	...
	Update  : Add variable varAnnexSectionTextLongColumnWidthCM in variables region
			  ...
			  Add display Annex
	          Add variable isScheduledNominalDisplayInAnnex: when 'true' display scheduled nominal in annex 
	          Add display Additional Information
	          Add display provisions
			  Add parameter pIsCalculationPeriodDatesDifferent in displayIrdProductTerms call template 
			  Add parameter pIsCalculationPeriodDatesDifferent in handleIrdStreams call template
			  Update parameter pIsNotionalDifferent in displayIrdProductTerms and handleIrdStreams call template
			  - Before: <xsl:with-param name="pIsNotionalDifferent" select="boolean($pIsNotionalDifferent)"/>
			  - After:  <xsl:with-param name="pIsNotionalDifferent" select="$IsNotionalDifferent"/>
	==============================================================	
	File    : Swap_ISDA_PDF.xslt
	Date    : 20.05.2009
	Author  : Gianmario SERIO
	Version : 2.3.0.3_1
	Description:
	==============================================================
	to do section:
	0: FxBarrier: Event Type: gestire il caso di 1 knock-in e 1 knock out 
	1: Testare il caso di rate cut-off dates (template displayFloatingRateCalculation). Per quali strumenti esistono queste dates?
	2: Personalizzazione del cliente: come gestire gli upgrade del web server. Il cliente modifica il file EFS che viene 
	   sovrascritto ad ogni installazione.
	4: Tutti i prodotti: migliorare l'aspetto grafico delle conferme: 
		- creare la nozione di “modello di conferma” (attraverso le globalVariables.xslt): “classic”, “modern”, ecc.	
	-->
  ...
  <xsl:param name="pProduct" select="'Swap'"/>
  <xsl:param name="pInstrument" select="'Vanilla Swap'"/>
  <xsl:param name="pInstrumentDescription" select="''"/>
  ...
  <!--
	...
	******************************************
	-->
  <!-- 
	isScheduledNominalDisplayInAnnex 
	when it is 'true': display scheduled nominal in annex
	when it is 'false': display scheduled nominal in Term, floating amount or fixed amount
	-->
  <xsl:variable name ="isScheduledNominalDisplayInAnnex">
    ...
    <fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format">
      ...
      <fo:page-sequence master-reference="A4" initial-page-number="1" font-family="{$varFontFamily}">
        ...
        <fo:flow flow-name="A4-body">
          ...
          <xsl:call-template name="displayIrdProductTerms">
            <xsl:with-param name="pIsSwaption"          select="false()"/>
            ...
            <xsl:with-param name="pIsNotionalDifferent" select="$isNotionalDifferent"/>
            <xsl:with-param name="pIsScheduledNominalDisplayInAnnex" select="$isScheduledNominalDisplayInAnnex"/>
            <xsl:with-param name="pIsUnderlyngSwap"     select="false()"/>
            <!-- GS 20110214: stub handle-->
            <xsl:with-param name="pIsTermDifferent"     select="$isTermDifferent"/>
            <xsl:with-param name="pIsStub"              select="$isStub"/>
            <xsl:with-param name="pIsAsynchronousStub"  select="$isAsynchronousStub"/>
          </xsl:call-template>

          <!-- Display streams (ex.: floating amounts, Fixed amounts...)-->
          <xsl:call-template name="handleIrdStreams">
            <xsl:with-param name="pStreams" select="//dataDocument/trade/swap/swapStream"/>
            <xsl:with-param name="pIsCrossCurrency" select="boolean($isCrossCurrency)"/>
            <xsl:with-param name="pIsNotionalDifferent" select="$isNotionalDifferent"/>
            <xsl:with-param name="pIsScheduledNominalDisplayInAnnex" select="$isScheduledNominalDisplayInAnnex"/>
            <!-- The tile in Floating and Fixing Amount Region is displayed -->
            <xsl:with-param name="pIsDisplayTitle"  select="true()"/>
            <!-- It displays details in Floating and Fixing Amount Region (detailled informations about payment dates (Rate Payer/Rate Option/Spread/Day Count/Payment Dates/Reset Dates/Compounding) -->
          ...
          <!-- Display exchange with parameter Initial -->
          ...
          <!-- Display exchange with parameter Final -->
          ...
          <!-- Display Provisions -->
          <xsl:if test="//dataDocument/trade/swap/earlyTerminationProvision or //dataDocument/trade/swap/cancelableProvision">
            <xsl:call-template name="handleProvisions">
              <xsl:with-param name="pInstrument" select="//dataDocument/trade/swap"/>
            </xsl:call-template>
          </xsl:if>

          <!-- Display AdditionalPayment (ex. Upfront) -->
          <xsl:if test="//dataDocument/trade/swap/additionalPayment">
            <xsl:call-template name="displayAdditionalPayment">
              <xsl:with-param name="pInstrument" select="//dataDocument/trade/swap"/>
            </xsl:call-template>
          </xsl:if>
          ...
          <xsl:if test="$isScheduledNominalDisplayInAnnex = 'true'">
            <xsl:call-template name="displayAnnex">
              <xsl:with-param name="pStream" select="//dataDocument/trade/swap/swapStream"/>
              <xsl:with-param name="pIsNotionalDifferent" select="$isNotionalDifferent"/>
            </xsl:call-template>
          </xsl:if>
  ...
  <xsl:variable name="varProductSectionTableWidthCM">
    <xsl:value-of select="$gVarProductSectionTableWidthCM" />
  </xsl:variable>
